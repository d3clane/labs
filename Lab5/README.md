...
![knuth](https://github.com/d3clane/labs/blob/Lab5/Lab5/ReadmeAssets/imgs/Knuth.png)

Битовое представление выглядит хорошо, скорее из-за того, что он переполняется (то есть берется модуль по 2^64) + берется модуль по размеру хеш-таблицы, но все равно получается хуже, чем взятие по модулю и метод умножения с константой Кнута. 

Взятие по модулю и метод умножение с константой Кнута - равномерные хеш-функции.
...
Битовое представление приведенного в int распределено в разные кучки - последствие того, что числа как отрицательные, так и положительные. При этом отрицательные числа имеют очень большое битовое представление.

Просто битовое представление тоже не очень. Возможно, дело в том, что числа лежат в узком диапазоне, экспонента лежит в узком диапазоне.

Извлечение мантиссы неплохой хеш, так как мантисса - 23 бита, то значений достаточно, чтобы заполнять всю хеш-таблицу. Для очень больших размеров таблиц (> 2^23) ничего начиная с какого-то номера заполняться не будет. 
...
Произведение мантиссы на экспоненту - не самый равномерный хеш. Полагаю, что он хороший в основном благодаря мантиссе, а экспонента, наоборот, ухудшает распределение.
...
Битовое инта быстрее чем битовое представление float. Полагаю из-за того, что в float заполнены верхние биты (экспонента) всегда, то есть операций много, пока число не занулится. При переводе в инт для чисел [0; 10] число зануляется за 4 сдвига, для [-10; 0] за все 32, в среднем получается лучше.
